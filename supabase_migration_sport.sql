...
CREATE TABLE IF NOT EXISTS public.activities (
  ...
  activity_type TEXT NOT NULL CHECK (activity_type IN ('run', 'walk', 'trail', 'cycle')),
  ...
  stopped_time INTEGER DEFAULT 0, -- in seconds
  ...
  max_altitude NUMERIC DEFAULT 0,
  elevation_gain NUMERIC DEFAULT 0,
  route_coordinates JSONB DEFAULT '[]'::jsonb, -- Array of { latitude, longitude }
  splits JSONB DEFAULT '[]'::jsonb, -- Array of { lapNumber, distanceKm, splitTimeSeconds, splitPaceFormatted, elevationDeltaMeters, avgCadenceSpm }
...
-- 3.1 Idempotent column additions for existing databases
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS moving_time INTEGER DEFAULT 0;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS elapsed_time INTEGER DEFAULT 0;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS stopped_time INTEGER DEFAULT 0;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS avg_cadence INTEGER DEFAULT 0;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS max_altitude NUMERIC DEFAULT 0;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS elevation_gain NUMERIC DEFAULT 0;
ALTER TABLE public.activities ADD COLUMN IF NOT EXISTS splits JSONB DEFAULT '[]'::jsonb;

-- 3.2 Update check constraint to include 'trail'
DO $$
BEGIN
  ALTER TABLE public.activities DROP CONSTRAINT IF EXISTS activities_activity_type_check;
  ALTER TABLE public.activities ADD CONSTRAINT activities_activity_type_check CHECK (activity_type IN ('run', 'walk', 'trail', 'cycle'));
EXCEPTION
